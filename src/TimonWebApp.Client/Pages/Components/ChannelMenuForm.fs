...
let validateChannelForm (channelForm) =
    ...
    all <| fun t -> {
        name = validateName t "Name" channelForm.name
    }
...
let update (timonService: TimonService) (message: Message) (model: Model) =
    let validateChannelForced form =
        ...
        {model with channelForm = form; errorsValidateChannelForm = Some mapResults}

    let validateChannel form =
        match model.errorsValidateChannelForm with
        | None  ->
            {model with channelForm = form}
        | Some _ -> validateChannelForced form

    match message, model with
    | SetChannelFormField ("name", value), _ ->
        {model.channelForm with name = value.Trim()} |> validateChannel, Cmd.none
    | ValidateChannel, _ ->
        model.channelForm |> validateChannelForced, Cmd.ofMsg (AddChannel)
    | ChannelAdded _, _ ->
        let channelForm = { model.channelForm with name = "" }
        { model with isAddingChannel = false; channelForm = channelForm }, Cmd.ofMsg NotifyChannelAdded
    | ToggleChannelForm, _ ->
        { model with isAddingChannel = not model.isAddingChannel }, Cmd.none
    ...
    | AddChannel, _ ->
        ...
        let cmd = Cmd.ofAsync
                    createChannel (timonService, payload)
                    ChannelAdded
                    raise
        model, cmd
    | _, _ -> model, Cmd.none
...
type Component() =
    ...
    override _.View model dispatch =
        let formFieldItem = Controls.inputAdd "Add new channel" Mdi.``mdi-pound`` model.errorsValidateChannelForm (Some "name")

        let inputCallback = fun v -> dispatch (SetChannelFormField("name",v ))
        let buttonAction = (fun _ -> dispatch ValidateChannel)

        let inputBox, icon =
            match model.isAddingChannel with
            | true -> (formFieldItem "Name" model.channelForm.name inputCallback buttonAction, Mdi.``mdi-chevron-up-circle-outline``)
            | false -> (empty, Mdi.``mdi-chevron-down-circle-outline``)
        ...
        | AuthState.Success ->
            ComponentsTemplate.AddChannelForm()
                    .Icon(icon)
                    .ChannelInput(inputBox)
                    .Elt()
        | _ ->
            ComponentsTemplate.AddChannelForm().Elt()
