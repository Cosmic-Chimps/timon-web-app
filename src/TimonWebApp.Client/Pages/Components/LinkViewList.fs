...
let validateTagForm (tagForm) =
    ...
    all <| fun t -> {
        tags = validateTag t "Tags" tagForm.tags
    }
...
let update (timonService: TimonService) (message: Message) (model: Model) =
    ...
    let validateTagForced (linkForm: LinkViewValidationForm) form  =
        ...
        {linkForm with tagForm = form; errorValidateForm = Some mapResults }

    let validateTag (validateTagForm: Result<TagForm,Map<string,string list>> option) (linkForm: LinkViewValidationForm) form =
        match validateTagForm with
        | None  ->
            {linkForm with tagForm = form }
        | Some _ -> validateTagForced linkForm form

    match message, model with
    | SetTagFormField ("tags", value, linkForm), _ ->
        let linkViewValidationForm = { linkForm.tagForm with tags = value.Trim() }
    ...
    | TagsAdded (linkId, _), _ ->
        let linkForm = model.links
                        |> Array.find (fun lf -> lf.view.Link.Id = linkId)

        let tagForm = { linkForm.tagForm with tags = ""}

        let newLf = { linkForm with tagForm = tagForm; isTagFormOpen = false; errorValidateForm = None }

        let linksMapAdding = updateLink newLf

        { model with links = linksMapAdding; }, Cmd.ofMsg NotifyTagsAdded

    | ToggleTagForm (linkForm), _ ->
        let linkForm' = { linkForm with isTagFormOpen = not linkForm.isTagFormOpen }
...
type Component()=
    ...
    override _.View model dispatch =
        forEach model.links (fun l ->
            let formFieldItem = Controls.inputAdd "Add new tag or more separated by commas" Mdi.``mdi-tag-outline`` l.errorValidateForm None

            let inputCallback = fun v -> dispatch (SetTagFormField("tags", v, l ))
            let buttonAction = (fun _ -> dispatch ( ValidateTag l))

            let inputBox, icon =
                match l.isTagFormOpen with
                | true -> (formFieldItem "Tags" l.tagForm.tags inputCallback buttonAction, Mdi.``mdi-close-circle-outline``)
            ...
            let iconNode =
                a [
                    ...
                    i [ attr.``class`` <| String.concat " " [ "mdi"; icon ] ] []
                ]
